# Sextant Tutorial 1: Building an Aging Model

## Overview

In this tutorial, you'll learn how to create a simple aging model with Sextant. We'll start with a CSV population file, write a configuration that ages everyone by one year, run the simulation, and analyze the output.

## Prerequisites

- Sextant binary downloaded and in your PATH (or in the current directory)
...
- Run a Sextant simulation
- Understand the basic structure of a Sextant configuration

**Important Note:** You don't need to be a programmer to use Sextant! We're using **Lua**, a simple and readable scripting language, for our models. Think of it as writing instructions in plain English:
...
The Lua script is the **brain** of your model. It tells Sextant **what** to do with your population data. Think of it this way:
...
Without the Lua script, Sextant wouldn't know:
...
Traditional microsimulation systems require you to edit and recompile the source code to change model behavior. With Sextant, you just edit the Lua script in the YAML file:
...
| **Embedded** | Lua is designed to be embedded in applications like Sextant. It just works. |
...
For Sextant, you only need to know a handful of Lua concepts:
...
**Note about `area_id`:** This column is used by Sextant's streaming mode to process populations area-by-area. For now, we'll use the simpler **Bulk Mode** which loads everything into memory at once.

---

## Step 2: Understanding YAML

YAML (YAML Ain't Markup Language) is a human-readable data format that Sextant uses for configuration.
...
**Note about `streaming_mode: false`:** We're using **Bulk Mode** in this tutorial. This loads the entire population into memory at once, which is simpler and faster for small populations. For large populations (1M+), Streaming Mode processes area-by-area.

---

## Step 4: Run the Simulation

Now run Sextant with your configuration:

```bash
# If sextant is in your PATH
sextant config_aging.yaml

# Or if sextant is in the current directory
./sextant config_aging.yaml
```
...
**Why is `id_column` required?** Every individual in your population needs a unique identifier. This is how Sextant keeps track of each person throughout the simulation. When people age, die, or have children, Sextant uses this ID to ensure it's updating the right person. Without a unique ID column, Sextant wouldn't be able to distinguish between individuals or link family members together. The `id_column` tells Sextant which column in your CSV contains these unique identifiers. Ordering the output by this column is a secondary benefit that makes it easier to compare results across different runs.
...
For **large populations** (1M+ people), you can use **Streaming Mode** (`streaming_mode: true`). This processes the population area-by-area, keeping memory usage low regardless of population size.
...
Each individual model is described as a Lua function. All model functions are called **`transition`** functions. This is a requirement of Sextant.
...
Each year, Sextant runs your `transition` functions to apply these changes to your population. That's why they're called transition functions - they describe how people **transition** from one state to another.

### How Sextant Finds Your Transition Function

When Sextant runs your model, it needs to know which function to execute. Rather than guessing, Sextant looks for a function with the exact name `transition`. This is similar to how:
...
By requiring a specific function name, Sextant can reliably find and run your model every year.

### What Happens If You Use a Different Name?

If you call your model function something else, Sextant won't find it and will throw an error:
...
**The short answer:** When Sextant loads your CSV, it creates a table (Lua's version of a dictionary/object) for each person. The column names become the **keys**, and the values become the **values**.
...
**2. Sextant loads each row as a Lua table:**

When Sextant reads row 1, it creates a table like this:
...
1. Sextant loads the CSV and creates a `person` table
...
8. ✅ Learned about Bulk Mode

You now know the basic workflow for using Sextant.
